 ...
 * is a suggestion, not a done deal: each one starts checked, but nothing is marked Done until
 * "Mark Done" is clicked, and any single one can be dismissed (unchecked and dropped from the
        ...
        {candidates.length === 0 ? (
          <p className={styles.empty}>
            {result.candidates.length === 0
              ? `Checked ${result.consideredCount} not-yet-Done shelf game${result.consideredCount === 1 ? '' : 's'} with a linked Steam app - nothing's 100%'d that isn't already marked Done.`
          ...
          <>
            <p className={styles.hint}>
              ...
              {candidates.length === 1 ? "isn't" : "aren't"} marked Done yet (checked {result.consideredCount}). Pick which to
            ...
            <div className={styles.actions}>
              ...
              <button type="button" className={styles.applyButton} onClick={handleApply} disabled={applying || selectedIds.size === 0}>
                {applying ? 'Marking Done…' : `Mark Done (${selectedIds.size})`}
              </button>
            </div>
          </>
        )}
